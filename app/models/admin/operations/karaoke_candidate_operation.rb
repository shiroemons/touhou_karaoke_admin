  ...
  module Operations
    class KaraokeCandidateOperation < BaseOperation
      ...
      def fetch_dam_song(progress: nil)
        ...
        Admin::ProgressReporter.report(progress:, percentage: 25, status: 'DAM候補追加中', label: '指定URLからDAM候補を取得しています')
        DamSong.fetch_dam_song(url)
        Admin::ProgressReporter.report(progress:, percentage: 96, status: 'DAM候補追加中', label: 'DAM候補の保存が完了しました')
      ...
      def fetch_joysound_detail(progress: nil)
        ...
        Admin::ProgressReporter.report(progress:, percentage: 25, status: 'JOYSOUND候補追加中', label: '指定URLからJOYSOUND候補を取得しています')
        JoysoundSong.fetch_joysound_song_direct(url:)
        Admin::ProgressReporter.report(progress:, percentage: 96, status: 'JOYSOUND候補追加中', label: 'JOYSOUND候補の保存が完了しました')
