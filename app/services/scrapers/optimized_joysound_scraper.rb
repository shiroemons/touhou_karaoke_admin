  ...
  class OptimizedJoysoundScraper < JoysoundScraper
    ...
    def update_delivery_models(song, new_delivery_model_ids)
      ActiveRecord::Base.transaction do
        ...
        if ids_to_add.any?
          ...
          SongsKaraokeDeliveryModel.insert_all(records) # rubocop:disable Rails/SkipsModelValidations
        end
      end
    ...
    def batch_update_delivery_models(songs_with_models)
      ActiveRecord::Base.transaction do
        ...
        # バルクインサート
        SongsKaraokeDeliveryModel.insert_all(records_to_insert) if records_to_insert.any? # rubocop:disable Rails/SkipsModelValidations
      end
    end
