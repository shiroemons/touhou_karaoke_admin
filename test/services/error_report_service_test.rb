...
class ErrorReportServiceTest < ActiveSupport::TestCase
  test 'collects errors with record url and exception context' do
    ...
    record = create_song
    exception = StandardError.new('タイムアウト')
    exception.set_backtrace(%w[line1 line2 line3 line4])

    service.add_error(type: :network, message: '取得中にタイムアウトしました', record:, url: record.url, exception:)
  ...
  test 'generates summary details and recommendations' do
    service = ErrorReportService.new
    11.times { service.add_error(type: :network, message: '取得中にタイムアウトしました') }
    6.times { service.add_error(type: :validation, message: '入力値が不正です') }
  ...
  test 'exports errors to csv and log text' do
    ...
    csv = File.read(path)
    assert_includes csv, '発生日時,種類,メッセージ,レコード種別,レコードID,URL,例外クラス'
    assert_includes csv, '入力エラー'

    log = service.to_log
    assert_includes log, '=== エラーレポート ==='
